...
function BOTCHED.FUNC.DermaColorRequest( text, title, default, confirmText, confirmFunc, cancelText, cancelFunc )
    ...
    local mixerEntry = vgui.Create( "DColorMixer", popup )
    mixerEntry:Dock( TOP )
	mixerEntry:DockMargin( 10, 0, 10, 0 )
	mixerEntry:SetTall( 100 )
    mixerEntry:SetAlphaBar( true )
    mixerEntry:SetWangs( true )
    mixerEntry:SetPalette( false )
    mixerEntry:SetColor( default ) 
    ...
    createButton( confirmText, function()
        if( confirmFunc ) then confirmFunc( mixerEntry:GetColor() ) end
    end )
    createButton( cancelText, cancelFunc )

	popup:SetExtraHeight( textArea:GetTall()+25+mixerEntry:GetTall()+buttonBack:GetTall()+10 )
end
