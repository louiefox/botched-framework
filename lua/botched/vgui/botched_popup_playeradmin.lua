...
function PANEL:Init()
    ...
    self.mainPanel.Paint = function( self2, w, h )
        BOTCHED.FUNC.BeginShadow( "popup_playeradmin" )
        BOTCHED.FUNC.SetShadowSize( "popup_playeradmin", w, h )
        local x, y = self2:LocalToScreen( 0, 0 )
        draw.RoundedBox( 8, x, y, w, h, BOTCHED.FUNC.GetTheme( 1 ) )			
        BOTCHED.FUNC.EndShadow( "popup_playeradmin", x, y, 1, 2, 2, 255, 0, 0, false )
    end
...
function PANEL:SetSteamID64( steamID64 )
    ...
    if( BOTCHED.CONFIG.GACHA ) then
        ...
        actions["givegempackage"] = {
            ...
            DoClick = function()
                local options = {}
                for k, v in pairs( BOTCHED.CONFIG.GACHA.GemStore ) do
                    options[k] = v.Name
                end

                BOTCHED.FUNC.DermaComboRequest( "What package would you like to give?", "GIVE GEM PACKAGE", options, "", false, "Continue", function( value, key )
