...
function PANEL:Init()
    ...
    self.mainPanel.Paint = function( self2, w, h )
        BOTCHED.FUNC.BeginShadow( "popup_cost" )
        BOTCHED.FUNC.SetShadowSize( "popup_cost", w, h )
        local x, y = self2:LocalToScreen( 0, 0 )
        draw.RoundedBox( 8, x, y, w, h, BOTCHED.FUNC.GetTheme( 1 ) )			
        BOTCHED.FUNC.EndShadow( "popup_cost", x, y, 1, 2, 2, 255, 0, 0, false )
    end
...
function PANEL:SetCostTable( costTable, doClick, cantAffordFunc )
    ...
    for k, v in pairs( costTable.Items or {} ) do
        local itemConfig = BOTCHED.CONFIG.LOCKER.Items[k]
