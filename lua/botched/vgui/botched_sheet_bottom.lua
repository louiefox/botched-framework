...
function PANEL:Init()
    ...
    self.navigationPanel:SetTall( BOTCHED.FUNC.ScreenScale( 50 ) )
...
function PANEL:AddPage( title, iconMat, panel, id, openFunc )
    ...
    if( title ) then
        ...
        local borderSize = math.ceil( BOTCHED.FUNC.ScreenScale( 5 ) )
        ...
        pageButton.Paint = function( self2, w, h )
            ...
            BOTCHED.FUNC.DrawPartialRoundedBoxEx( 8, 0, h-borderSize, w, borderSize, BOTCHED.FUNC.GetTheme( 3, self2.alpha ), false, 16, false, h-borderSize-(16-borderSize), false, false, pageKey == 1, pageKey == #self.pages )

            local textColor = BOTCHED.FUNC.GetTheme( 4, 75+(180*(self2.alpha/150)) )
            surface.SetDrawColor( textColor )
            surface.SetMaterial( iconMat )
            surface.DrawTexturedRect( (w/2)-(textX/2)-iconSize-borderSize, (h/2)-(iconSize/2), iconSize, iconSize )
        ...
        pageButton.DoClick = function()
            if( self.CanSwitch and not self:CanSwitch() ) then return end
...
vgui.Register( "botched_sheet_bottom", PANEL, "DPanel" )